...
const AddMiscellaneousExpense = () => {
  ...
  const totalCost = useMemo(() => {
    ...
    return fields.reduce((sum, k) => sum + (Number(formData[k]) || 0), 0);
  }, [formData]);

  const estimatedPKR = useMemo(() => {
    return (Number(totalCost) || 0) * (Number(formData.exchangeRate) || 1);
  }, [totalCost, formData.exchangeRate]);

  useEffect(() => {
    const load = async () => {
      setIsLoading(true);
      try {
        const currencyRes = await userRequest.get('/currencies');
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await userRequest.post('/expenses/miscellaneous', payload);
  ...
  return (
    <div className="p-4">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">Add Miscellaneous Expense</h1>
      </div>
