...
const AddWarehouseExpense = () => {
  ...
  const sumUtilities = (u) =>
    (Number(u.electricity) || 0) + (Number(u.water) || 0) + (Number(u.gas) || 0) + (Number(u.internet) || 0);

  const calcTotalCost = () => {
    const total =
      (Number(formData.rentAmount) || 0) +
      sumUtilities(formData.utilities) +
      (Number(formData.staffSalaries) || 0) +
      (Number(formData.securityCost) || 0) +
      (Number(formData.repairsCost) || 0) +
  ...
  useEffect(() => {
    const load = async () => {
      setIsLoading(true);
      try {
        const [whRes, currencyRes, productsRes] = await Promise.all([
        ...
        // Default currency: try to pick PKR
  ...
  const handleUtilityChange = (e) => {
    const { name, value } = e.target; // electricity, water, gas, internet
    setFormData(prev => ({
      ...prev,
      utilities: {
        ...prev.utilities,
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        linkedStock: formData.linkedStock || undefined,
        expensePeriod: {
      ...
      await userRequest.post('/expenses/warehouse', payload);
      toast.success('Warehouse expense added successfully');
      navigate('/expenses/warehouse');
    } catch (error) {
      console.error(error);
      toast.error(error.response?.data?.message || 'Failed to add warehouse expense');
    } finally {
  ...
  return (
    <div className="p-4">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">Add Warehouse Expense</h1>
      </div>
            ...
            <CardBody className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  label="Start Date"
                  labelPlacement="outside"
                  value={formData.expensePeriod.startDate}
                ...
                <Input
                  ...
                  label="End Date"
                  labelPlacement="outside"
                  value={formData.expensePeriod.endDate}
                  onChange={handlePeriodChange}
                />
              </div>
            ...
            <CardBody className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Select
                  ...
                  labelPlacement="outside"
                  selectedKeys={
                    formData.linkedStock ? [formData.linkedStock] : []
                  }
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      linkedStock: e.target.value,
                    }))
                  }
                >
                  {products.map((p) => (
...
export default AddWarehouseExpense;
