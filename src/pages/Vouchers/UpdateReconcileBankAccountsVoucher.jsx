...
const UpdateReconcileBankAccountsVoucher = ({ voucherId, onBack }) => {
  ...
  useEffect(() => {
    const fetchVoucher = async () => {
      try {
        setIsLoading(true);
        const response = await userRequest.get(`/reconcile-bank-accounts-vouchers/${voucherId}`);
        ...
        setFormData({
          ...
          openingBalance: voucher.openingBalance?.toString() || '',
          closingBalance: voucher.closingBalance?.toString() || '',
          bookBalance: voucher.bookBalance?.toString() || '',
          statementBalance: voucher.statementBalance?.toString() || '',
          entries: voucher.entries && voucher.entries.length > 0
            ? voucher.entries.map((entry) => ({
                statementDate: formatDateForInput(entry.statementDate),
                statementDescription: entry.statementDescription || '',
                statementAmount: entry.statementAmount?.toString() || '',
                statementType: entry.statementType || 'credit',
          ...
          outstandingDeposits: voucher.outstandingDeposits?.toString() || '',
          outstandingWithdrawals: voucher.outstandingWithdrawals?.toString() || '',
          outstandingChecks: voucher.outstandingChecks?.toString() || '',
          bankCharges: voucher.bankCharges?.toString() || '',
          interestEarned: voucher.interestEarned?.toString() || '',
          currency: voucher.currency?._id || voucher.currency || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await userRequest.put(`/reconcile-bank-accounts-vouchers/${voucherId}`, formDataToSend, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      toast.success('Reconcile Bank Accounts Voucher updated successfully');
      ...
      toast.error(
        error.response?.data?.message || 'Failed to update reconcile bank accounts voucher'
      );
    } finally {
          ...
          <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-3xl md:text-4xl font-bold text-white">
                    Update Reconcile Bank Accounts Voucher
                  </h1>
                  <p className="text-teal-100 text-sm md:text-base mt-1">
                    {voucherInfo?.voucherNumber || voucherInfo?.referCode || 'Edit reconciliation details'}
                  </p>
                </div>
              </div>
            </div>
            <div className="hidden md:flex items-center gap-3">
              <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                <p className="text-white text-xs font-medium">Difference</p>
                  ...
                  <div className="mb-6 pb-4 border-b-2 border-gray-200">
                    <div className="flex items-center gap-3">
                      <div className="bg-teal-100 p-2 rounded-lg">
                        <FaEdit className="text-teal-600 text-xl" />
                      </div>
                      <div>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              {attachment.type?.startsWith('image/') ? (
                                ...
                                <div className="w-16 h-16 bg-teal-100 rounded flex items-center justify-center">
...
export default UpdateReconcileBankAccountsVoucher;
