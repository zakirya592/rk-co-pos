...
const UpdateBankAccountTransferVoucher = ({ voucherId, onBack }) => {
  ...
  useEffect(() => {
    const fetchVoucher = async () => {
      try {
        setIsLoading(true);
        const response = await userRequest.get(`/bank-account-transfer-vouchers/${voucherId}`);
        ...
        setFormData({
          ...
          transferMethod: voucher.transferMethod || '',
          transferFee: voucher.transferFee?.toString() || '',
          referenceNumber: voucher.referenceNumber || '',
          fromBankTransactionId: voucher.fromBankTransactionId || '',
          toBankTransactionId: voucher.toBankTransactionId || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await userRequest.put(`/bank-account-transfer-vouchers/${voucherId}`, formDataToSend, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      toast.success('Bank Account Transfer Voucher updated successfully');
      ...
      toast.error(
        error.response?.data?.message || 'Failed to update bank account transfer voucher'
      );
    } finally {
          ...
          <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-3xl md:text-4xl font-bold text-white">
                    Update Bank Account Transfer Voucher
                  </h1>
                  <p className="text-teal-100 text-sm md:text-base mt-1">
                    {voucherInfo?.voucherNumber || voucherInfo?.referCode || 'Edit transfer details'}
                  </p>
                </div>
              </div>
            </div>
            <div className="hidden md:flex items-center gap-3">
              <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                <p className="text-white text-xs font-medium">Transfer Amount</p>
...
export default UpdateBankAccountTransferVoucher;
