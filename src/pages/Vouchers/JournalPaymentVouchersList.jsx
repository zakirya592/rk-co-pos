...
const JournalPaymentVouchersList = ({ onAddNew, onView, onEdit }) => {
  ...
  const fetchVouchers = async () => {
    const response = await userRequest.get('/journal-payment-vouchers');
  ...
  const handleDelete = async (voucher) => {
    ...
    if (result.isConfirmed) {
      try {
        await userRequest.delete(`/journal-payment-vouchers/${voucher._id}`);
  ...
  const generatePrintContent = (voucher) => {
    return `
      ...
      <html>
        <head>
          <title>Cash Payment Voucher - ${voucher.voucherNumber || voucher.referCode}</title>
          <style>
            @media print {
              @page {
                size: A4;
          ...
          <div class="voucher-info">
            ...
            <div class="info-section">
              ...
              <div class="info-row">
                ...
                <span class="info-value">${voucher.shop?.name || voucher.shop || 'N/A'}</span>
              </div>
              ` : ''}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
          ...
          <div className="xl:col-span-3">
            ...
            <div className="overflow-x-auto">
              <Table
                aria-label="Journal Payment Vouchers table"
                selectionMode="none"
                classNames={{
                  wrapper: 'min-h-[500px]',
                }}
                ...
                >
                  ...
                  <TableCell>
                    <div className="font-semibold text-green-600">
                      {formatCurrency(
                        voucher.entries?.reduce((sum, e) => sum + (parseFloat(e.debit) || 0), 0) || 0,
                        voucher.currency
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="font-semibold text-red-600">
                      {formatCurrency(
                        voucher.entries?.reduce((sum, e) => sum + (parseFloat(e.credit) || 0), 0) || 0,
                        voucher.currency
                      )}
                    </div>
                  </TableCell>
            ...
            {filteredVouchers.length === 0 && vouchers.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-500 mb-4">
                  Get started by creating your first journal payment voucher
                </p>
                <Button
                  color="primary"
                  startContent={<FaPlus />}
            ...
            {isLoadingDetails ? (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-purple-50 p-4 rounded-lg border border-blue-200">
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg border border-green-200">
                ...
                {selectedVoucher.entries && selectedVoucher.entries.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
                      ...
                      Journal Entries ({selectedVoucher.entries.length})
                    </p>
                    <div className="space-y-3">
                          ...
                          <CardBody className="p-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <p className="font-semibold text-gray-900">{entry.accountName || 'N/A'}</p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Account Model</p>
                                <p className="font-medium text-gray-700">{entry.accountModel || 'N/A'}</p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Debit</p>
                                <p className="font-semibold text-green-600">
                                  {formatCurrency(entry.debit || 0, selectedVoucher.currency)}
                                </p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Credit</p>
                                <p className="font-semibold text-red-600">
                                  {formatCurrency(entry.credit || 0, selectedVoucher.currency)}
                                </p>
                              </div>
                              {entry.description && (
                    ...
                    <Card className="mt-4 bg-gradient-to-r from-purple-50 to-indigo-50">
                      <CardBody className="p-4">
                        <div className="grid grid-cols-3 gap-4 text-center">
                          <div>
                            ...
                            <p className="text-lg font-bold text-green-600">
                              {formatCurrency(
                                selectedVoucher.entries.reduce((sum, e) => sum + (parseFloat(e.debit) || 0), 0),
                          ...
                          <div>
                            ...
                            <p className="text-lg font-bold text-red-600">
                              {formatCurrency(
                                selectedVoucher.entries.reduce((sum, e) => sum + (parseFloat(e.credit) || 0), 0),
                                selectedVoucher.currency
                              )}
                            </p>
                          </div>
                ...
                {/* Currency Info */}
                <div className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white p-6 rounded-xl mt-4">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-purple-100 text-sm font-medium mb-1">Currency</p>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/20">
                    <div>
                      <p className="text-purple-100 text-xs mb-1">Exchange Rate</p>
                      <p className="font-semibold">
                        {selectedVoucher.currencyExchangeRate || '1'}
                      </p>
                    </div>
                    <div>
                      <p className="text-purple-100 text-xs mb-1">Symbol</p>
                      <p className="font-semibold">
                            ...
                            <div className="flex items-start gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <a
                                  ...
                                  className="text-xs text-purple-600 hover:text-blue-800 font-medium inline-flex items-center gap-1"
                                >
                                  View Full Size →
                                </a>
                              </div>
...
export default JournalPaymentVouchersList;
