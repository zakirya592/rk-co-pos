...
const AddOperationalExpense = () => {
  ...
  const calcTotalCost = () => {
    const total =
      (Number(formData.employeeSalaries) || 0) +
      (Number(formData.officeRent) || 0) +
      (Number(formData.officeSupplies) || 0) +
      (Number(formData.stationery) || 0) +
      (Number(formData.softwareExpenses) || 0) +
  ...
  const handleUtilityChange = (e) => {
    const { name, value } = e.target; // electricity, internet, phone, water
    setFormData((prev) => ({
      ...prev,
      utilities: {
        ...prev.utilities,
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await userRequest.post('/expenses/operational', payload);
      toast.success('Operational expense added successfully');
      navigate('/expenses/operational');
    } catch (error) {
      console.error(error);
      toast.error(error.response?.data?.message || 'Failed to add operational expense');
    } finally {
  ...
  return (
    <div className="p-4">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">Add Operational Expense</h1>
      </div>
              ...
              >
                {users.map((u) => (
                  <SelectItem key={u._id} value={u._id}>
                    {u.name || u.fullName || u.email || u.username || u._id}
                  </SelectItem>
                ))}
              </Select>
          ...
          <Card>
            <CardBody className="space-y-4">
              ...
              <div className="text-right text-sm text-gray-600">
                Estimated total: <span className="font-semibold">{calcTotalCost().toLocaleString()}</span>
              </div>
            </CardBody>
          </Card>
        </div>
...
export default AddOperationalExpense;
