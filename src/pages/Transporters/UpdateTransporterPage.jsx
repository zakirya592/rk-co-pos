...
const UpdateTransporterPage = () => {
    ...
    const fetchTransporter = async () => {
      try {
        ...
        reset({
          ...
          country: transporter.country || 'Pakistan',
          commissionRate: transporter.commissionRate,
          paymentTerms: transporter.paymentTerms || 'advance',
          rating: transporter.rating?.toString() || '0',
        //   isActive: transporter.isActive?.toString() || 'true',
        });

        // Set vehicle types and routes
        setSelectedVehicleTypes(transporter.vehicleTypes || []);
        setRoutes(transporter.routes?.length > 0 ? 
          transporter.routes : 
          [{ origin: '', destination: '', estimatedDays: '', ratePerKg: '' }]
        );
      } catch (error) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await userRequest.put(`/transporters/${id}`, transporterData);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Update Transporter</h1>
        <Button
          color="primary"
          variant="light"
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {vehicleTypes.map((type) => (
                    <Chip
                      key={type.value}
                      color={selectedVehicleTypes.includes(type.value) ? 'primary' : 'default'}
                      variant={selectedVehicleTypes.includes(type.value) ? 'solid' : 'bordered'}
                ...
                {selectedVehicleTypes.length === 0 && (
                  <p className="text-xs text-red-500 mt-1">Please select at least one vehicle type</p>
                )}
              </div>
              ...
              {routes.map((route, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleRouteChange(index, 'estimatedDays', e.target.value)}
                    placeholder="e.g., 2"
                  ...
                  <Input
                    ...
                    onChange={(e) => handleRouteChange(index, 'ratePerKg', e.target.value)}
        ...
        <div className="flex justify-end gap-4">
          ...
          >
            {isSubmitting ? 'Updating...' : 'Update Transporter'}
          </Button>
        </div>
      </form>
    </div>
...
export default UpdateTransporterPage;
